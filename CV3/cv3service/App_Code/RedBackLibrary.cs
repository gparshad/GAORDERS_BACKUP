...
public class RedBackLibrary
{
            ...
            foreach (CV3Library.Order.ShipTo s in o.ShipTos)
            {
                if (isRedBackAvailable == true)
                {
                    try
                    {
                        ...
                        if (brandCode == "52" || brandCode == "9" || brandCode == "10" || brandCode == "17")
                        {
                            if (s.Message.Trim() != "")
                            {
                                //((RedProperty)rb.Property("Comments")).Value = "Gift Message: " + s.Message;
								((RedProperty)rb.Property("Comments")).Value = s.Message;
                            }
                        }
                        else
                        {
    ...
    public string SendToRedbackWithDefaultKeycode(ref List<CV3Library.Order> orders, string brandCode, string ordPrefix, string keycode, bool token)
    {
        ...
        foreach (CV3Library.Order o in orders)
        {
            ...
            foreach (CV3Library.Order.ShipTo s in o.ShipTos)
            {
                ...
                {
                    shipToCount++;
                    string shipToPrefix = shipToCount > 1 ? "" + shipToCount.ToString() : "0";
                    try
                    {
                        rb = new RedObject();
                        rb.Open3(RedBackAccount, "OPM:Order_New");
 			    ...
 			    if (o.Mobile != "" && (brandCode == "4" || brandCode == "7" || brandCode == "5" || brandCode == "1") && (o.Promotionsms == "m" || o.Promotionsms == "t"))
                            {
                                ((RedProperty)rb.Property("MobilePhone")).Value = o.Mobile;
                            }
                        ...
                        ((RedProperty)rb.Property("WebReference")).Value = "CV3" + ordPrefix + o.OrderID + shipToPrefix;
                        if (o.PromoCode == "")
    ...
    public string FetchOrder(string custNumber, string custZip, string brandCode, string orderNumber)
    {
        ...
        rsp.Append("\t\t<ShipStatus>" + ((RedProperty)rb.Property("DetStatus")).Value + "</ShipStatus>\n");
